...
impl CheckpointEngine {
    ...
    pub fn verify_checkpoint<B: CheckpointBackend>(
        ...
        for component in &stored.bundle.manifest.components {
            ...
            let readback_verified =
                match backend.verify_readback(&component.component_id, &component.content_sha256) {
                    Ok(readback) => {
    ...
    /// Evaluate restore compatibility independently from checkpoint integrity verification.
    ///
        ...
        for component in &stored.bundle.manifest.components {
            ...
            match target_map.get(&component.producer_provider_instance_ref) {
                Some(provider_target)
                    if provider_target.target_provider_generation
                        > component.provider_generation => {}
    ...
    /// Restore a verified bundle into an exact target after a separately proven,
    ...
    pub fn restore<B: CheckpointBackend>(
        ...
        validate_restore_target(&stored, &target)?;
        validate_compatibility_decision(bundle_id, &target, compatibility_decision, now_unix_ms)?;
    ...
    pub fn verify_recovery(
        ...
        let verifier_independent =
            !verifier_ref.trim().is_empty() && verifier_ref != restore.executor_ref;
        ...
        let missing_evidence = evidence_refs.is_empty()
            ...
            || postconditions
                .iter()
                .any(|item| item.key.trim().is_empty() || item.evidence_refs.is_empty());
...
fn preflight_restore(stored: &StoredBundle, target: &RestoreTarget) -> Result<(), CheckpointError> {
    ...
    for component in &stored.bundle.manifest.components {
        ...
        {
            return Err(CheckpointError::CompatibilityUnsatisfied(
                requirement.clone(),
            ));
        }
...
fn provider_target_for<'a>(
    ...
    target
        ...
        .ok_or_else(|| {
            CheckpointError::MissingProviderTarget(component.producer_provider_instance_ref.clone())
        })
}
...
fn validate_request(request: &CheckpointRequest) -> Result<(), CheckpointError> {
    for (name, value) in [
        ...
        (
            ...
            request.workspace_revision_ref.as_str(),
        ),
        (
...
fn consistency_satisfies(actual: Consistency, requested: Consistency) -> bool {
    match requested {
        ...
        Consistency::CrashConsistent => {
            matches!(
                actual,
                Consistency::Consistent | Consistency::CrashConsistent
            )
        }
...
fn validate_restore_target(
    ...
    for provider_target in &target.provider_targets {
        if provider_target
            .source_provider_instance_ref
            .trim()
            .is_empty()
            || provider_target
                .target_provider_instance_ref
                .trim()
                .is_empty()
            || provider_target.target_provider_generation == 0
        {
            return Err(CheckpointError::InvalidRestoreTarget("provider_targets"));
        }
...
fn validate_compatibility_decision(
    bundle_id: &str,
    target: &RestoreTarget,
