...
Path('crates/ptah-checkpoint/tests/a13.rs').write_text(r'''//! A13 checkpoint/restart/recovery authoritative acceptance and adversarial corpus.
...
fn incompatible_unknown_expired_and_rebound_decisions_cannot_mutate() {
    ...
    let restore_target = target();
    let unknown = engine
        .evaluate_restore_compatibility(&bundle.bundle_id, &restore_target, Vec::new(), NOW, VALID_UNTIL)
