...
#[test]
fn public_authorization_rejects_blank_explicit_approval() {
    let current = snapshot();
    for (kind, target) in [
        (ControlKind::SubmitUpgradeActivity, "advisory-1"),
        (ControlKind::AcceptWorkerResult, "worker-1"),
    ] {
        let mut control = request(&current, kind, target);
...
#[test]
fn mismatched_workspace_selection_cannot_dispatch_protected_controls() {
    let app = include_str!("../web/app.js");
    assert!(app.contains("button.disabled = mismatch"));
    let submit_start = app
        .find("async function submit(button)")
        .expect("submit function");
    let submit = &app[submit_start..];
    let mismatch_guard = submit
        .find("if (selector.value !== authority.workspace_id)")
        .expect("workspace mismatch guard");
    let control_fetch = submit
        .find("const response = await fetch('/api/control'")
        .expect("control request");
    assert!(mismatch_guard < control_fetch);
}

#[test]
fn authorization_result_survives_canonical_refresh() {
    let app = include_str!("../web/app.js");
    let submit_start = app
        .find("async function submit(button)")
        .expect("submit function");
    let submit = &app[submit_start..];
    let refresh = submit.find("await refresh();").expect("canonical refresh");
    let authorized_status = submit
        .find("status.textContent = `${kind} authorized for dispatch as ${result.request_id}; this is not completion.`")
        .expect("authorization result status");
    assert!(refresh < authorized_status);
}
